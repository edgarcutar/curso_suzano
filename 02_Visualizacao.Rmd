...
Existem muitos materiais e cursos sobre como criar gráficos com o ggplot2. Basta um simples google sobre o assunto ou problema que você tem e rapidamente vai aparecer uma pergunta semelhante e várias rzonaclimaticaostas assunto. Como padrão, a documentação do pacote pode ser consultada, bem como diversos livros sobre o assunto. Recomendamos o site do pacotes [ggplot2](https://ggplot2.tidyverse.org/) e o [livro do autor](https://www.amazon.com.br/dp/B01GVCRF6M/)
...
O primeiro argumento da função  `ggplot()` é um `data.frame`, e seu segundo argumento é `aes()`. 
...
p <- ggplot(inv, aes(x = idinvr, y = vcccr))
p
```
...
Cada layer tem uma base de dados. Você pode indicar os *dados* na função principal `ggplot()` ou nos layers zonaclimaticaecíficos. O mesmo ocorre para a definição das *dimensões* com a função `aes()`.

## A camada de *geom*etria
...
Caso você queira alterar o formato do ponto, basta zonaclimaticaecificar no argumento indicado.
...
Nesse gráfico, foi ajustada uma regressão linear com um intervalo de confiança de 95%.
...
O ggplot2 gerou automaticamente uma paleta de cores para os dados e criou uma legenda. Tal como acontece com tudo, a paleta de cores também é ajustável, o que será discutido em mais detalhe abaixo. O padrão de cores do ggplot2 é bastante inteligente. Cada cor é equidistante em torno de um círculo de cor HSL, e têm igual luminância. A ideia é que nenhuma categoria tenha maior destaque que outra. Um ponto negativo é que ela pode ser um problema para leitores daltônicos.
...
ggplot(resumo_zonaclimatica, aes(zonaclimatica, idinvr)) +
  geom_col()
```

Se quisermos colorir as barras, o primeiro instinto seria provavelmente para definir `color = zonaclimatica`. Mas este argumento considera apenas o contorno das figuras bidimensionais.

```{r}
ggplot(resumo_zonaclimatica, aes(zonaclimatica, idinvr, color = zonaclimatica)) +
  geom_col()
```

O preenchimento é feito utilizando o argumento `fill = zonaclimatica`.

```{r}
ggplot(resumo_zonaclimatica, aes(zonaclimatica, idinvr, fill = zonaclimatica)) +
  geom_col()
```
...
```{r}
ggplot(inv, aes(vcccr, fill = zonaclimatica)) +
...
```{r}
ggplot(resumo_zonaclimatica, aes(zonaclimatica, ima7ccr, label = round(ima7ccr))) +
    geom_col() +
    geom_label() +
    annotate("text", x = 5, y = 120, label = "Anotação")
```
...
Visualizar a distribuição de frequências também é um recurso muito interessante para entender os nossos dados. Podemos fazê-lo através de um histograma eu de gráfico de densidade de frequências. Como exemplo iremos visualizar a distribuição do peso dos participantes do curso.
...
Podemos identificar a distribuição do zonaclimatica através de cores.
...
Entretanto o padrão da função `geom_density()` é usar `position = "identity"`. Isso significa que pode haver (como de fato acontece) sobreposição das curvas. Uma alternativa rápida é colocar transparência no preenchimento para poder ver através das curvas.
...
ggplot(inv, aes(idinvr, vcccr)) +
  geom_point() +
  scale_x_continuous(breaks = seq(0, 10, 1), labels = paste(seq(0, 10, 1), "anos"))
```
...
Caso você queria colocar suas proprias cores, terá de fazer isso manualmente. Eu recomendo o site http://colorbrewer2.org/. A partir do código hexadecimal, podemos colocar as cores que queremos com a função `scale_[color|fill]_manual()`. Os [templates de PowerPoint da Suzano](https://www.centraldamarcasuzano.com.br/pt/materiais-do-dia-a-dia#materiais-de-papelaria) tem os hexadecimais das cores das paletas oficiais em um slide.
